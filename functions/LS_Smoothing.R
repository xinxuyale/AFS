...
LSS <- function(order, q = 0.95, d=0.25, qs=0.97) {
  ...
  Q_qs <- quantile(abs(order$intens[2:n]-order$intens[1:(n-1)]), qs)
  Q_99 <- quantile(abs(order$intens[2:n]-order$intens[1:(n-1)]), .99)
  while(Q_99 > Q_qs){
    mask <- which(abs(order$intens[2:n]-order$intens[1:(n-1)])>Q_99)
    mask <- mask + 1
    order <- order[-mask,]
    n <- dim(order)[1]
    Q_99 <- quantile(abs(order$intens[2:n]-order$intens[1:(n-1)]), .99)
  ...
  B2 <- predict(final_fit, data.frame(wv = wavelength))
  
  # Return the smoothed lab source spectrum.
  return(cbind(wavelength, B2/u))
}
